...
const wx_http = new Request()
wx_http.setConfig((config) => { /* 设置全局配置 */
  config.baseUrl = wxUrl /* 根域名不同 */
  config.header = {
    ...config.header
  }
  return config
...
wx_http.interceptor.request((config, cancel) => { /* 请求之前拦截器 */
  tip.alert(config.baseUrl)
  config.header = {
    ...config.header,
     'X-Access-Token':getTokenStorage(),
...
wx_http.interceptor.response(async (response) => { /* 请求之后拦截器 */
  // if (response.data.code !== 200) { // 服务端返回的状态码不等于200，则reject()
  //   return Promise.reject(response)
  // }
  return response
